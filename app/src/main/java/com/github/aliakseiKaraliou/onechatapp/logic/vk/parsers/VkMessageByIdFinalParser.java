...
public class VkMessageByIdFinalParser implements IParser<String, List<IMessage>> {
    ...
    @Nullable
    @Override
    public List<IMessage> parse(String s) {
        List<IMessage> messageList = new ArrayList<>();
        try {
            JSONArray messages = new JSONObject(s).getJSONObject(Constants.Json.RESPONSE).getJSONArray(Constants.Json.ITEMS);
            for (int i = 0; i < messages.length(); i++) {
                ...
                if (currentObject.has(Constants.Json.CHAT_ID)) {
                    ...
                    IChat chat = (IChat) VkReceiverStorage.get(chatId);
