    ...
    protected List<IUser> doInBackground(String s){
        ...
        try {
            ...
            for (int i = 0; i < responceArray.length(); i++) {
                ...
                result.add(new com.example.korol.onechatapp.logic.vk.entities.VkUser(responceObject.getInt("id"), responceObject.getString("first_name"), responceObject.getString("last_name"), responceObject.getString("photo")));
            }
        } catch (Exception e) {
            e.printStackTrace();
            result.add(null);
